...
public class MoveValidator {
    ...
    // This gets updated to the most recent moves that involves the capturing of a
    // piece
    private static ArrayList<MoveInformation> captureMoves = new ArrayList<>();

    public static Pair<Boolean, MoveInformation> validateMove(Move playerMove, BoardView boardView,
            boolean whiteMoving) {
    ...
    /**
     * Checks if a given position occupies a cell on the board, see if it occupies a
     * space on the board
     *
    ...
    /**
     * Checks if a move is properly diagonal going forward for a regular piece
     * (single step) Checks if properly diagonal forward or backward for a king
     * piece (single step) ( A single diagonal move )
     *
    ...
    private static boolean moveIsSingleProperDiagonal(BoardView boardView, Move move, boolean whiteMove) {
        ...
        // If both the col offset and the row offset are either 1 or -1 then it is a
        // diagonal move
    ...
    /**
     * Checks if the move being made is going properly diagonally forward (Any
     * amount of steps) (any piece) Checks if the move being made is going properly
     * diagonally any direction (Any amount of steps) (king) Will be used to
     * validate jump moves
     *
    ...
    public static boolean isMultipleJumpMove(BoardView boardView, Move move, boolean whiteMove) {
        ...
        // Iterate along the "vectors" looking for a repeating pattern of [enemy_piece,
        // blank_space]
        ...
        if (whiteMove)
            colorToCapture = Piece.Color.RED;
        ...
        while (cur_row != move.end.row && cur_col != move.end.cell) {
            ...
            if (cycleIndex == 0) {
                if (cur_piece == null)
                    return false;
                if (cur_piece.getColor() != colorToCapture)
                    return false;
                MoveInformation moveInformation = new MoveInformation(move, cur_piece,
                        Position.makePosition(cur_row, cur_col));
    ...
    public boolean hasPossibleSimpleMoves(BoardView boardView, Position current_position) {
        ...
        // Don't waste time looking for moves if there is no piece here and make it
        // known
        if (current_piece == null) {
        ...
        for (int row_offset = -1; row_offset <= 1; row_offset++) {
            for (int col_offset = -1; col_offset <= 1; col_offset++) {
                ...
                Position artificial_ending_position = Position.makePosition(current_position.row,
                        current_position.cell);
                Move move_to_check = Move.createMove(current_position, artificial_ending_position);
                if (!moveIsOnBoard(move_to_check)) {
                    continue;
                }
     ...
     * @return True, if there is an available single jump move for the piece at this
     *         position
     */
    public static boolean hasPossibleSingleJumpMoves(BoardView boardView, Position current_position) {
        ...
        for (int row_offset = -2; row_offset <= 2; row_offset++) {
            for (int col_offset = -2; col_offset <= 2; col_offset++) {
                // Create moves and validate if they are single jump moves
                Position artificial_ending_position = Position.makePosition(current_position.row + row_offset,
                        current_position.cell + col_offset);
                Move move_to_check = Move.createMove(current_position, artificial_ending_position);
    ...
    public static boolean anyJumpMoves(BoardView boardView, boolean whiteMove) {
        ...
        for (int row = 0; row < NUM_ROWS; row++) {
            for (int col = 0; col < NUM_COLS; col++) {
                ...
                Piece current_piece = boardView.getSpace(current_position).getPiece();
                // If the current piece is null don't even bother checking and move on to next
                // piece
                if (current_piece == null) {
                    continue;
                }
